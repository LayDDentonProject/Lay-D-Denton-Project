...
function PreviousPortrait()
{
	...
	if (bFemaleEnabled == false || PortraitIndex%2 == 0)
	{
...
function SaveSettings()
{
	...
	else
	{
		...
		log(Self$" "$Player$" setting bMadeFemale, Human(Player) == "$Human(Player));
		if (Human(Player) != None)
		{
			Human(Player).bMadeFemale = (PortraitIndex % 2 == 1);
			log(Self$" "$Player$" setting bMadeFemale to "$(PortraitIndex % 2 == 1)$", result: "$Human(Player).bMadeFemale);
		}
	}
}
...
function CreateLDDPCheckboxes()
{
	...
	if( !bFemaleEnabled ) {
		MorpheusCheckbox.SetPos(5000+MorpheusCheckboxPos.X, MorpheusCheckboxPos.Y);
		MorpheusCheckbox.SetToggle(true);
	}
